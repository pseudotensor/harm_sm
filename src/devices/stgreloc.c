...
int x, y;			/* destination */
{
   int dx, dy;				/* offset from previous point */
...
/***************************************************************/
/*
 * Draw a sm_dot. Note that we have just moved to the current sm_location, so if
 * startmark is null we needn't move there again
 */
int
stg_dot(x, y)
int x, y;			/* location */
{
   if(g_sg->sg_startmark[0] == '\0' && g_sg->sg_endmark[0] == '\0') {
      return(-1);
   } else {
