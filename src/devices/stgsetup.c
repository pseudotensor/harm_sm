...
char *g_xy;			/* pointer to coord encoding string */
int g_in, g_out;		/* file descriptors for device */
int g_xc, g_yc;			/* current window position */
 ...
 */
   ...
   if(initialise(g_tty,av[0]) < 0 ||	/* Initialize data structures*/
			scale() < 0) {	/* get SCREEN -> device coords */
      FREE(g_sg);
      FREE(g_tty);
      return(-1);
   }
/*
 * copy the arguments into g_sg
 */
   for(i = j = 0;i < ac;i++) {
 ...
 */
   ...
   g_xc = 0;				/* current device coordinates */
   g_yc = 0;
/*
