...
int c;					/* what shall I do ? */
{
   ...
   static unsigned char save_eotchar;
   static unsigned char save_eolchar;
#  if defined(POSIX_TTY)
   struct termios arg;
...
#  endif
	 save_eotchar = save_arg.c_cc[VMIN];
	 save_eolchar = save_arg.c_cc[VTIME];
#else
...
#if defined(POSIX_TTY) || defined(SYS_V)
      save_eotchar = arg.c_cc[VMIN];
      save_eolchar = arg.c_cc[VTIME];
...
#  endif
      ...
      REDRAW(fildes);			/* give the device a chance */
