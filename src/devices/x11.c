...
/*
 * process command line arguments, and look for an -device argument
 */
 ...
 */
	...
	(DoesBackingStore(ScreenOfDisplay(XDEV->display,screen)) == Always);
      ...
      icon_pixmap = XCreateBitmapFromData(XDEV->display,XDEV->root,(char *)icon_bitmap,
					  SM_ICON_XSIZE,SM_ICON_YSIZE);
 ...
 */
	 ...
	 wmhints.flags |= IconPositionHint;
      ...
      XStoreName(XDEV->display,XDEV->wind,title);
      XDEV->event_mask = ExposureMask | StructureNotifyMask;
...
int *x,*y;
{
   ...
   if(XDEV->curswind != 0) {
	 ...
	 int width = sm_image_cursor ?
	   (30+10)*XDEV->w_font + 2 : (19+10)*XDEV->w_font + 2;
 ...
 */
   ...
   while(XCheckMaskEvent(XDEV->display,reduced_mask,&event) ||
	 ...
	 size_x11();
	 if(XDEV->old_height != XDEV->height ||
...
XrmDatabase *rdb;			/* merged database */
{
   ...
   if(XDEV->display == NULL) {
      ...
      if((XDEV->display = XOpenDisplay(ptr)) == NULL) {
