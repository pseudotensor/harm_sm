...
char **av;
{
	...
	*ptr;
   FILE *fil;
   int greeting = 1,			/* greet user on startup */
       i,
...
init_version()
{
   ...
   if(m < 0 || m > 12) m = 0;
   sprintf(version_date,"%d%s %s %d",d,
	   ((d == 11 || d == 12 || d == 13) ? "th" :
	     d%10 == 1 ? "st" :
	     d%10 == 2 ? "nd" :
	     d%10 == 3 ? "rd" :
   ...
   *ptr = '\0';
   ...
   if((version_string =
       malloc(25 + strlen(version_name) + strlen(version_date))) == NULL) {
      msg("Can't allocate storage for version_string\n");
      return;
   }
   sprintf(version_string,"SM: version %s (%s)",version_name,version_date);
}
...
usage()
{
   ...
   static char *msgs[] = {
      ...
      "-u user         Intepret ~ as user's home directory in .sm search path",
