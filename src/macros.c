...
Void *nn;			/* old node */
{
   ...
   if((node->macro = malloc((unsigned)strlen(mmacro) + 1)) == NULL) {
      msg_1s("malloc returns NULL in mmake() for %s\n",name);
      free((char *)node);
      return(NULL);
   }
   (void)strcpy(node->macro,mmacro);
   if(mmacro[0] == cchar) {		/* skip initial comment */
...
FILE *ffil;				/* Stream containing macros */
{
   ...
   mend = macro + MACROSIZE - 1 - tabsize;
   fil = ffil;
   ...
   while(fgets(line,2*LSIZE,fil) != NULL) {
	 ...
	 if(*lptr == '\0' && *(lptr - 1) != '\n') { /* no closing \n */
	    ungetc(' ',fil);		/* so provide a fake space to persuade
					   SM to keep on reading the line */
	 }
      }
      if(*(mptr - 1) == '\n') *(mptr - 1) = '\0';
...
static int mindex = -1;			/* index to stack */
static MSTACK *mstack;			/* the stack itself */
static unsigned n_mstack = 0;		/* max number of elements on stack */
...
int nargs_min, nargs_max, arg_eol;
{
   ...
   int c,
       i,
       mnew;				/* new value of mindex */

   if((mnew = mindex + 1)  >= n_mstack) {
      n_mstack += DSTACKSIZE;
      if(mnew == 0) {
	 ...
	 if((mstack = (MSTACK *)realloc((char *)mstack,
      ...
      for(i = 0;i < n_mstack;i++) {
	 mstack[i].args[0] = mstack[i].name;
      }
   }
      
   (void)strcpy(mstack[mnew].name,name);
   mstack[mnew].nargs = nargs_max;
   mstack[mnew].local_var = mstack[mnew].local_vec = NULL;
   mstack[mnew].size_lvar = mstack[mnew].size_lvec = 0;
   if(nargs_max > 0) {
      if((mstack[mnew].args[1] = malloc((unsigned)nargs_max*ASIZE)) == NULL) {
	 ...
	 mstack[mnew].args[i] = mstack[mnew].args[1] + (i - 1)*ASIZE;
	 if(sm_interrupt) {			/* respond to ^C */
	    mstack[mnew].args[i][0] = '\0';
	 } else {
	    if(i <= nargs_min) {
	    ...
	    if(i == nargs_max && arg_eol) {
	       ...
	       if(c == EOF) {
		  *mstack[mnew].args[i] = '\0';
	       } else {
		  (void)unput(c);
		  (void)mscanline(mstack[mnew].args[i],ASIZE);
	       }
	    } else {
	       (void)mscanstr(mstack[mnew].args[i],ASIZE);
	    }
   ...
   if(mnew == mindex + 1) {
      mindex = mnew;
   } else {				/* popped stack while reading args */
      mstack[++mindex] = mstack[mnew];
      mstack[mnew].args[1] = NULL;		/* not really needed */
      mstack[mindex].args[0] = mstack[mindex].name; /* fix up pointer */
   }
}
