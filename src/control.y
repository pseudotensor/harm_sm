...
%type <charval>   number_or_word something string string_or_space
		  word_or_space
...
command : '\n'
	...
	{ in_graphics = 0; GFLUSH(); }
	 ...
	 { (void)sm_delimage(); }
      | IMAGE WORD
	 { (void)read_image($2,0.0,0.0,0.0,0.0); }
      | IMAGE WORD number number number number
	 { (void)read_image($2,$3,$4,$5,$6); }
      | IMAGE '(' INTEGER ',' INTEGER ')'
	 { create_image($3,$5,0.0,(float)$3 - 1,0.0,(float)$5 - 1); }
      | IMAGE '(' INTEGER ',' INTEGER ')' number number number number
	 { create_image($3,$5,$7,$8,$9,$10); }
      ...
      | LIMITS limit_pair limit_pair
	 {
	    ...
	    (void)sm_limits( $2.a, $2.b, $3.a, $3.b );
	 }
      ...
      | LWEIGHT number
	 {
      ...
      | MACRO WORD integer_or_space delim_str_list /* define macro */
         {
	    if($4.start == NULL) break;
            *($4.end) = '\0';		/* remove '\n' */
	    sprintf(buff,"%d",$3);
            (void)define_s($2,$4.start,buff);
	 ...
	 { (void)copy_vector($2,$4); }
      | LOCAL SET WORD { make_local_vector($<charval>3); } '=' set_expr
...
expr : delim_list			/* set a vector to a list */
	    ...
	    if($1 == NULL) break;
	    if(num_or_word($1->i_list[0]->str) == 0) {
	       type = VEC_STRING;
	    } else {
	       type = VEC_FLOAT;
	    }
	 ...
	 { vec_concat(&$1,&$3,&$$); }
      | WORD
	    ...
	    if($1[0] == '0') {		/* may be a hex or octal constant */
	       REAL val;
	       if((val = atof2($1)) == 0) {
		  if(!strcmp($1,"0x0")) { /* the only non-word == 0 */
		     ;
		  } else {
...
graphic : AXIS number number number number INTEGER INTEGER INTEGER
	 ...
	 { sm_dot(); }
	  ...
	  { gdraw($3,$4); }
      | ERASE
	  { (*devices[devnum].dev_erase)(); }	/* saying ERASE() won't work */
	 ...
	 { sm_ltype( 10 ); }
      ...
      | plot pexpr pexpr IF '(' expr ')'
	    ...
	    if($1 == CONNECT) {
	       (void)conn_if( $2.vec.f, $3.vec.f, $6.vec.f, npoint );
	    } else if($1 == HISTOGRAM) {
	       (void)histogram_if( $2.vec.f, $3.vec.f, $6.vec.f, npoint );
	    } else if($1 == POINTS) {
	       points_if( $2.vec.f, $3.vec.f, $6.vec.f, npoint );
	    } else {
	       msg("Unknown plotting command\n");
	    }
      ...
      | RELOCATE '(' number number ')'
	  { sm_grelocate($3,$4); }
      | fill INTEGER pexpr pexpr	/* Shade (or fill) a region */
	 {
	    int npoint;
	    ...
	    if($1 == 1) {
	       (void)sm_shade($2,$3.vec.f,$4.vec.f,npoint);
	    } else if($1 == 2) {
	       (void)shade_hist($2,$3.vec.f,$4.vec.f,npoint);
	    } else {
...
limit_pair : pexpr
	 {
	    ...
	    if(fabs(x_range) < 1e-20 && fabs(y_range) < 1e-20) {
	       min = 1e30; max = -1e30;
	       for(i = 0;i < $1.dimen;i++) {
	          if($1.vec.f[i] < NO_VALUE) { /* not a bad point */
		     if($1.vec.f[i] > max) {
		        max = $1.vec.f[i];
		     }
		     if($1.vec.f[i] < min) {
	       ...
	       sort_flt($1.vec.f,$1.dimen);

	       for(i = $1.dimen - 1;i >= 0;i--) { /* ignore bad points */
	          if($1.vec.f[i] < NO_VALUE) break;
	       }
	       if(i < 0) {
	       ...
	       if(i%2 == 0) {
	          ...
	          $$.med = ($1.vec.f[i/2] + $1.vec.f[i/2 + 1])/2;
	       }
	    }
	    diff = max - min;
	    if(diff == 0) {
...
number : sign FLOAT			/* accept any positive number */
         { $$ = $1 ? -$2 : $2; }
      | sign INTEGER
         { $$ = $1 ? -$2 : $2; }
      ...
      | WHATIS '(' something ')'	/* What is this? */
	 {
	    ...
	    if(tok == INTEGER || tok == FLOAT) {
	       ...
	       if(get_macro($3,(int *)NULL,(int *)NULL,(int *)NULL)
			         != NULL) i |= 02;	/* a macro */
	       if(*print_var($3) != '\0') i |= 04;	/* a variable */
...
s_expr : expr			/* a scalar expression */
	 {
	    ...
	    if($1.type != VEC_FLOAT && sm_verbose > 1) {
	       msg_1s("Error: %s is not an arithmetic vector\n",$1.name);
	       vec_free(&$1);
	       YYERROR;
	    }
	...
	{ $$ = $1; }
      | expr IF '(' expr ')' /* conditional assignment */
	    ...
	    for(i = 0,j = 0;i < $1.dimen;i++) {
	       if(i > $4.dimen || $4.vec.f[i]) { /* copy acceptable elements */
		  if($1.type == VEC_FLOAT) {
		     $1.vec.f[j] = $1.vec.f[i];
