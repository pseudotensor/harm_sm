...
int flag;			/* if 1, get value from image */
{
   ...
   if(key == 'e') {
      sm_relocate(ux,uy);
      (void)sprintf(buff,"relocate %g %g",ux,uy);
     ...
     *name_z;			/* optional name of intensity vector */
{
   ...
   msg("Enter a `e', `m', or `p' to mark a point, `q' to exit\n");
   sm_enable();
   ...
   for(i = 0;;i++) {
	 ...
	 key = CURSOR(&px, &py);	   	/* read the SCREEN position */
	 sm_disable();
	 if(key == -1) {		/* no cursor */
	    break;
	 }
