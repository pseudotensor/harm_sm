...
int ixa,iya,ixb,iyb;
{
   ...
   if(lltype == 0) {
      ...
      if((index = abs(lltype) - 1) >= sizeof(onoff)/sizeof(onoff[0])) {
	 msg_1d("Illegal ltype: %d\n",lltype); /* we can't get here, as this
						  is checked in ltype(),
						  but we may as well check */
	 return;
      }
      onofflen = onoff[index][0] + onoff[index][1] + onoff[index][2] +
							       onoff[index][3];
      start = (int)totdist % onofflen;
      for(istart = 0;istart < 4;istart++) {
	 if(start < onoff[index][istart]) break;
         start -= onoff[index][istart];
      }
   }
