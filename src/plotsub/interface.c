...
REAL *x1,*x2,*y1,*y2;
{
   read_image(STRING(name),*x1,*x2,*y1,*y2);
}
...
{
   if(*type == 1) {
      sm_shade(*delta,x,y,*nxy);
   } else if(*type == 2) {
      shade_hist(*delta,x,y,*nxy);
   } else {
      msg_1d("Unknown type of shading: %d\n",*type);
   }
}
...
int *key;
{
   ...
   *key = CURSOR(&px,&py);		/* read the position */
...
char *nname;
{
   char name[80];			/* writable version of nname */
   int dn;
   static int first = 1;
 ...
 */
   ...
   (void)strncpy(name,nname,sizeof(name)-1); /* nname may not be writable */
