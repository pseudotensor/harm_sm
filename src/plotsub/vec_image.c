...
/*
 * check that vectors are all floating
 */
   if((xi != NULL && xi->type != VEC_FLOAT) ||
      (yi != NULL && yi->type != VEC_FLOAT) || vals->type != VEC_FLOAT) {
      if(xi != NULL && xi->type != VEC_FLOAT) {
	 msg_1s("Vector %s is not arithmetic",xi->name);
	 yyerror("show where");
      }
      if(yi != NULL && yi->type != VEC_FLOAT) {
	 msg_1s("Vector %s is not arithmetic",yi->name);
	 yyerror("show where");
      }
      if(vals->type != VEC_FLOAT) {
