...
// JCM: Notice that for BASELOG=10, if big tick is at 1, small ticks are at 2,3,4,5,...9
// For BASELOG=100, however, if big tick is at 1, small ticks are at 1+i*delta from i=1,9 with delta =  (100-1+1)/10 = 10
// So in that case the small ticks are at 11,21,31,etc. and NOT 10,20,30 as one might think
//#define BASELOG 100.0
#define BASELOG 10.0
#define SMALLTICKSLOG (10.0)
 ...
 */
	 adiff = fabs(a2 - a1);
	 for(;;) {
	    diff = log10( adiff/ nlabel )/log10(BASELOG);
	    exp_val = diff;
 ...
 */
	    step = ticks*pow(BASELOG,(float)exp_val);
 ...
 */
	 ...
	 return;
      ...
      step = (ab == 0.0) ? BASELOG : (ab > 0.0 ? ab : -ab);
      // JCM: number of small ticks per big tick (assume SMALLTICKSLOG no matter BASELOG)
      smticks = step/BASELOG*SMALLTICKSLOG/(-as) + 0.5;
   }
...
int drawit;				/* if true, actually draw labels */
{
   ...
   // JON DEBUG:
   //   fprintf(stderr,"a1=%g a2=%g offset=%d lscale=%d step=%d\n",a1,a2,offset,lscale,step);
 ...
 */
   ...
   while(!sm_interrupt) {
      for(i = 0;i < smticks;i++) {
	// ~BASELOG*i/10
	value = tick_place + step*i/smticks; // ranges from 0 to step for small ticks
	 ...
	 if(as >= 0) {
	    ...
	    // i=0 always skipped
	    if(i==1){ // always is big=1
	      length = alen*(log10(value)/log10(BASELOG) + lscale/log10(BASELOG) - a1)/(a2 - a1);
	      //length = alen*(log10(value-10.0+1.0)/log10(BASELOG) + lscale/log10(BASELOG) - a1)/(a2 - a1);
	      //	      length = alen*(-log10(step*1.0/smticks)/log10(BASELOG) + log10(value)/log10(BASELOG) + lscale/log10(BASELOG) - a1)/(a2 - a1);
	    }
	    else if(i!=1){ 
	      //	      length = alen*(-1.0/log10(BASELOG) - log10(step*1.0/smticks)/log10(BASELOG) + log10(value)/log10(BASELOG) + lscale/log10(BASELOG) - a1)/(a2 - a1);
	      length = alen*( -log10(step*1.0/smticks)/log10(BASELOG) + log10(value)/log10(BASELOG) + lscale/log10(BASELOG) - a1)/(a2 - a1);
	    }
	 }
	 if(length < -1) {
	 ...
	 // JON DEBUG:
	 //	 fprintf(stderr,"i=%d smticks=%d step=%g :: value=%g :: tick_place=%g lscale=%d length/alen=%g\n",i,smticks,step,value,tick_place,lscale,length/alen);
	 //	 fprintf(stderr,"ax=%d ay=%d alen=%d\n",ax,ay,alen);
	 //	 fprintf(stderr,"first=%g second=%g\n",log10(value)/log10(BASELOG),lscale/log10(BASELOG));

      }// end for loop
      tick_place += step;  // increment big step
      if(as < 0.0) {
	 if(a2 > a1) {
	   if(tick_place >= 0.99999*BASELOG) { // check if should do next big tick section
	       tick_place = 0.0;
	       lscale+=log10(BASELOG);
	       if(ab < 0) step /= BASELOG;
	    }
	 } else {
	    if(tick_place < 0.1) {
	       tick_place = 1.0;
...
char *str;
{
   ...
   ystart -= sheight + eexpand*LABEL_OFFSET;
...
char *str;
{
   ...
   xstart -= sdepth + eexpand*LABEL_OFFSET;
...
int drawit;				/* actually draw label? */
{
    ...
    if (is_parallel) {			/* parallel to axis */
	  ...
	  if(is_clockwise) {		/* clockwise */
	     dy = eexpand*LABEL_OFFSET;
	  } else {				/* widdershins */
	     dy = -(eexpand*LABEL_OFFSET + sheight);
	  }
	  ...
	  draw_string(string);
       ...
       if(drawit) {
	  if(is_clockwise) {		/* clockwise */
	     dx = -slength - eexpand*LABEL_OFFSET;
	  } else {				/* widdershins */
	     dx = eexpand*LABEL_OFFSET;
	  }
	  dy = -sheight/2;
...
char *string;
{
   ...
   if(aangle <= 45.) {     /*   x axis   */
      ...
      if (x_num_fmt[0] != '\0' ) {	/* user supplied the x-axis format */
         (void)sprintf(string,x_num_fmt,value*pow(BASELOG,(float)lscale));
         return;
      } 
   } else {
      lexp = ylexp;
      ...
      if(y_num_fmt[0] != '\0' ) {	/* the user supplied y-axis format */
         (void)sprintf( string, y_num_fmt,value*pow(BASELOG,(float)lscale));
         return;
      }
   }
   if(TeX < 0) {
