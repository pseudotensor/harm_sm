...
static void axial P_ARGS(( REAL *, REAL *, int, double, REAL *, REAL *));
static void front_plane P_ARGS(( REAL *, REAL *, int, double ));
static void plane P_ARGS(( REAL *, REAL *, int, double ));
static void projective P_ARGS((REAL *,REAL *,int, double, REAL *, REAL *));
static void (*project) P_ARGS((REAL *,REAL *,int, double, REAL *, REAL *)) =
								    projective;
...
/* 
 * We use negative l (distance) for axial projection in stead of type.
 * type /= 10;
 ...
 */
   ...
   (*project)(edge_x, edge_z, 4, front_y, frontedge_x, frontedge_z);
   (*project)(edge_x, edge_z, 4, back_y,  backedge_x,  backedge_z );
 ...
 */
   for(j = ifront_y;!sm_interrupt && j != iback_y + diy;j += diy) {
	 ...
	 for(i = 0;i < nxz;i++) {
      ...
      if(j == ifront_y) {
	 front_plane(x,z,nxz,y[ifront_y]);
      } else {
	 plane(x,z,nxz,y[j]);
      }
   }
...
REAL *x_p,*z_p;				/* projected x, z values */
{
   ...
   for(i = 0;i < n;i++) {
      ...
      pz = (z[i] < fz1) ? fz1 : ((z[i] > fz2) ? fz2 : z[i]);
