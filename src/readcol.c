...
#define LINESIZE 100000			/* maximum size of line to read */
...
static long seek_ptr = -1;		/* where are we in the file? */
static int fortran_file = UNDEFINED;
static char *getline P_ARGS(( char *, int, FILE * ));
static char *getline_phys P_ARGS(( char *, int, FILE * ));
 ...
 */
   for(i = 0;i < nvec;i++) {
      if(vec_malloc(vector[i],DSPACE) < 0) {
...
VECTOR **vector;
{
   for(;*fmt != '\0';fmt++) {
	 ...
	 if(*++fmt == '*') continue;
	 switch (*fmt) {
	  case 'd': case 'e': case 'f': case 'g': case 'n': case 'o': case 'x':
...
int l1;
{
   ...
   if((fil = fopen(file,mode)) != NULL) {
      if(use_seek) {
	 if(seek_ptr >= 0 && l1 >= line_ctr) { /* we can save time by seeking*/
	    if(sm_verbose > 1) {
	       msg_1d("Seeking to start of line %d",line_ctr);
	       msg_1s(" of file %s\n",file);
	    }
	    if(fseek(fil,seek_ptr,0) < 0) {
   ...
   while(*line != '\0') {
	 ...
	 switch (c = *fmt++) {
	  case 'd': case 'o': case 'x':
	    ...
	    base = (c == 'o') ? 8 : (c == 'd' ? 10 : 16);
	    while(isdigit(*line)) {
	       ival = base*ival + (*line++ - '0');
	    }
	    if(is_negative) ival = -ival;
	    if(!skip) {
	       vector[ivec++]->vec.f[nvec] = ival;
	       nread++;
	    }
	    break;
	  ...
	  case 's':
	    if(end == NULL) {		/* unknown width */
		  ...
		  for(ptr = vector[ivec++]->vec.s.s[nvec];
		      *line != '\0' && !isspace(*line);) {
		     *ptr++ = *line++;
		  }
		  *ptr = '\0';
		  ...
		  strcpy(vector[ivec++]->vec.s.s[nvec],line);
		  nread++;
	       }
	    }
	    break;
	  case '[':
	    ...
	    if(skip) {
	       while(*line != '\0' && (*is_allowed)[*line]) line++;
	    } else {
	       for(ptr = vector[ivec++]->vec.s.s[nvec];
		   *line != '\0' && (*is_allowed)[*line];) {
		  *ptr++ = *line++;
	       }
	       *ptr = '\0';
	       nread++;
...
FILE *fil;
{
   ...
   if(use_seek) seek_ptr = ftell(fil); /* remember start of line */
   
   if(getline_phys(line,line_size,fil) == NULL) {
      seek_ptr = -1;
      return(NULL);
   }
/*
 ...
 */
   ...
   while(line[len - 2] == '\\' && line[len - 1] == '\n') {
      char *ret = getline_phys(&line[len - 2],line_size-len+2,fil);
      len += strlen(&line[len]);
      if(ret == NULL) {
	 line[len - 2] = '\n';
	 line[len - 1] = '\0';
 ...
 */
static char *
getline_phys( line, line_size, file_ptr )
char *line;
int line_size;
FILE *file_ptr;
{
...
VECTOR *vector;				/* vector to read into */
{
   ...
   REAL value;				/* value to read */
   int j,
       n_elem;				/* number of elements on vector */
   unsigned size;			/* current size of vector->vec.f */
