...
char *av[];
{
   char empty[2],			/* empty[0] => no pixels turned on */
   ...
   int color,				/* is this a colour plot? */
       ...
       nbits,				/* num. of significant bits per char */
 ...
 */
   if((color = ttygetb(g_tty,"CL")) != 0) { /* a colour device */
      ...
      mask[0] = 0;			/* this'll be reset */
   } else {
      if((nc = ttygets(g_tty,"BP",mask,NBITS*NBYTES + 2)) <= 0) {
	 nc = NBITS*nb;
	 for(i = 0;i < nc;i++) {
	    mask[i] = 1 << (i%NBITS);
	 }
 ...
 */
   ...
   if((page = (char **)malloc(nrow*sizeof(char *))) == NULL) {
      fprintf(stderr,"rasterise: can't allocate storage for page\n");
      exit(EXIT_BAD);
   }
   for(i = 0;i < nrow;i++) {
      if((page[i] = (char *)malloc((unsigned)row_by)) == NULL) {
         fprintf(stderr,"rasterise: cannot allocate %dth row\n",i);
         exit(EXIT_BAD);
      }
   }

   for(i = 0;i < row_by;i++) {	/* fill first row with `empty' */
      page[0][i] = *empty;
   }
   for(i = 1;i < nrow;i++) {
 ...
 */
	 if(mr) {
      ...
      if(sm_verbose) {
         fprintf(stderr,"Read %ld vectors\n",nvec);
      }
   }
   if(!std_in) close(g_in);
   if(remove_file) {
 ...
 */
   if(color) {
      ...
      char *row;			/* storage for an expanded row */
      
      row_by *= 3;			/* ROW() writes row_by bytes */
      if((row = (char *)malloc((unsigned)row_by)) == NULL) {
	 fprintf(stderr,"rasterise: can't allocate storage for row\n");
	 exit(EXIT_BAD);
      }

      for(i = nrow - 1;i >= 0;i--) {
	 for(c = j = 0;j < row_by;j += 3,c++) {
	    col = page[i][c];
	    row[j] = red[col];
	    row[j+1] = green[col];
	    row[j+2] = blue[col];
